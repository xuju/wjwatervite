...
import PopupModal3D from '../PopupModal3D';
...
export const renderDom = (view: SceneView | MapView, attr: PointAttr, div: HTMLElement) => {
...
const YWZTScene = () => {
  ...
  const view = useView();
  const setView = useSetRecoilState(sceneAtom);
  ...
  const featureWatchHandle = useRef<IHandle>();
  let clickHandel = useRef<IHandle | null>();
  const { state: layerData, dispatch: layerDispatch } = useLayerContext();
  // 选择的图层
  const selectedLayers = useLayerList();
  const { run: getLayersRun } = useRequest(() => service.post(businessApi.get3DLayer, ztParam), {
    manual: true
  });
  ...
  useEffect(() => {
    ...
    if (selectedLayers) {
      clickHandel.current = view?.on('click', (event) => {
        let { mapPoint } = event;
        let promiseAll: any[] = [];
  ...
  useEffect(() => {
    if (results) {
      ...
      //单独处理FeatureServer,添加图层名称
      for (const key in result) {
        const current = result[key];
        const layerName = current.graphic.layer.title;

        const attributes = current.graphic.attributes as Object;
        if (!attributes.hasOwnProperty('layerName')) {
          current.graphic.attributes.layerName = layerName;
        }
        if (!attributes.hasOwnProperty('mapPoint')) {
          current.graphic.attributes.mapPoint = current.mapPoint;
        }
      }
      ...
      if (featureShowSmallPopup.includes(layerName)) {
        ...
        if (view) {
          ...
          if (featureWatchHandle.current) {
            featureWatchHandle.current.remove();
          }
          featureWatchHandle.current = view.watch('extent', () => {
  ...
  return (
    <div className="YWZTScene">
      ...
      <PopupModal3D data={isBigAttrPopup} setResponse={setIsBigAttrPopup} />
